		...
		for(var/datum/gas_mixture/gas in gases)
			...
			if(gas.trace_gases.len)
				for(var/datum/gas/trace_gas in gas.trace_gases)
					...
					air_transient.add_tracegas(trace_gas, trace_gas.moles)
			...
			for(var/datum/gas_mixture/gas in gases)
				...
				if(air_transient.trace_gases.len)
					for(var/datum/gas/trace_gas in air_transient.trace_gases)
						...
						gas.set_tracegas(trace_gas,trace_gas.moles*gas.volume/air_transient.volume)
	...
	for(var/datum/gas_mixture/gas in gases)
		...
		if(gas.trace_gases.len)
			for(var/datum/gas/trace_gas in gas.trace_gases)
				...
				if(!corresponding.is_tracegas(trace_gas))
					corresponding = new trace_gas.type()
		...
		for(var/datum/gas_mixture/gas in gases)
			...
			if(total_trace_gases.len)
				for(var/datum/gas/trace_gas in total_trace_gases)
					...
					gas.add_tracegas(trace_gas,trace_gas.moles*gas.volume/total_volume)
			gas.update_values()
