		...
		if(istype(target,/obj))
			...
			if(!O.anchored)
				if(cargo_holder.cargo.len < cargo_holder.cargo_capacity)
					...
					set_ready_state(0)
					chassis.use_power(energy_drain / (getupgrade("mech_clamp_efficiency") * getupgrade("mech_equip_efficiency")))
					O.anchored = 1
					var/T = chassis.loc
			...
			if(chassis.occupant.a_intent == "hurt")
				M.take_overall_damage(dam_force * getupgrade("mech_clamp_damage"))
				M.adjustOxyLoss(round((dam_force/2) * getupgrade("mech_clamp_suffocate")))
				M.updatehealth()
...
/obj/item/mecha_parts/mecha_equipment/tool/drill
	...
	action(atom/target)
		if(!action_checks(target)) return
		set_ready_state(0)
		chassis.use_power(energy_drain / (getupgrade("mech_drill_efficiency") * getupgrade("mech_equip_efficiency")))
			...
			if(0)
				if (istype(target, /turf/simulated/wall))
					...
					if(do_after_cooldown(target))
						...
						chassis.give_power(energy_drain * (getupgrade("mech_rcd_efficiency") * getupgrade("mech_equip_efficiency")))
			...
			if(1)
				if(istype(target, /turf/space))
					...
					if(do_after_cooldown(target))
						...
						playsound(target, 'Deconstruct.ogg', 50, 1)
						chassis.spark_system.start()
						chassis.use_power(energy_drain*2 / (getupgrade("mech_rcd_efficiency") * getupgrade("mech_equip_efficiency")))
				else if(istype(target, /turf/simulated/floor))
...
/obj/item/mecha_parts/mecha_equipment/teleporter
	...
	action(atom/target)
		...
		if(T)
			set_ready_state(0)
			chassis.use_power(energy_drain / (getupgrade("mech_teleporter_efficiency") * getupgrade("mech_equip_efficiency")))
...
/obj/item/mecha_parts/mecha_equipment/wormhole_generator
	...
	action(atom/target)
		...
		if(!target_turf)
			return
		chassis.use_power(energy_drain / (getupgrade("mech_portal_efficiency") *  getupgrade("mech_equip_efficiency")))
		set_ready_state(0)
			...
			if(1)
				...
				if(!locked)
					...
					if(locked in view(chassis))
						...
						send_byjax(chassis.occupant,"exosuit.browser","\ref[src]",src.get_equip_info())
						set_ready_state(0)
						chassis.use_power(energy_drain / (getupgrade("mech_gravipult_efficiency") * getupgrade("mech_equip_efficiency")))
						do_after_cooldown()
					else
						locked = null
				...
				for(var/atom/movable/A in atoms)
					...
					spawn(0)
						...
						for(var/i=0 to iter)
							...
							sleep(2)
				set_ready_state(0)
				chassis.use_power(energy_drain / (getupgrade("mech_rcd_efficiency") * getupgrade("mech_equip_efficiency")))
				do_after_cooldown()
		return
...
/datum/global_iterator/mecha_repair_droid
	...
	process(var/obj/item/mecha_parts/mecha_equipment/repair_droid/RD as obj)
		if(!RD.chassis)
			...
			return
		var/health_boost = RD.health_boost * (getupgrade("mech_repair_healpower"))
		var/repaired = 0
		...
		if(health_boost<0 || RD.chassis.health < initial(RD.chassis.health))
			...
			repaired = 1
		if(repaired)
			if(RD.chassis.use_power(RD.energy_drain / (getupgrade("mech_repair_efficiency") * getupgrade("mech_equip_efficiency"))))
		...
		if(cur_charge<ER.chassis.cell.maxcharge)
			...
			if(A)
				...
				if(pow_chan)
					...
					ER.chassis.give_power(delta * getupgrade("mech_energyrelay_drainpower"))
					A.master.use_power(delta*ER.coeff * getupgrade("mech_energyrelay_drainpower"), pow_chan)
		return
...
/datum/global_iterator/mecha_generator
	...
	process(var/obj/item/mecha_parts/mecha_equipment/generator/EG)
		...
		if(isnull(cur_charge))
			...
			stop()
			return 0
		var/use_fuel = EG.fuel_per_cycle_idle * getupgrade("mech_generator_fueluse")
		if(cur_charge<EG.chassis.cell.maxcharge)
			use_fuel = EG.fuel_per_cycle_active * getupgrade("mech_generator_fueluse")
			EG.chassis.give_power(EG.power_per_cycle * getupgrade("mech_generator_efficiency"))
...
/datum/global_iterator/mech_sleeper
	...
	process(var/obj/item/mecha_parts/mecha_equipment/tool/sleeper/S)
		...
		S.chassis.use_power(S.energy_drain / (getupgrade("mech_sleeper_efficiency") * getupgrade("mech_equip_efficiency")))
		S.update_equip_info()
		return
...
/obj/item/mecha_parts/mecha_equipment/tool/syringe_gun
	...
	action(atom/movable/target)
		...
		if(reagents.total_volume<=0)
			...
			return
		set_ready_state(0)
		chassis.use_power(energy_drain / (getupgrade("mech_syringe_efficiency") * getupgrade("mech_equip_efficiency")))
...
/datum/global_iterator/mech_synth
	...
	process(var/obj/item/mecha_parts/mecha_equipment/tool/syringe_gun/S)
		...
		for(var/reagent in S.processed_reagents)
			...
			S.chassis.use_power(energy_drain / (getupgrade("mech_syringe_efficiency") * getupgrade("mech_equip_efficiency")))
		return 1
