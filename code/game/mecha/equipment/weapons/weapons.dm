...
/obj/item/mecha_parts/mecha_equipment/weapon/energy
	...
	action(target)
		...
		chassis.use_power(energy_drain / getupgrade("mech_weapon_efficiency"))
		A.fired()
	...
	action(target)
		...
		for(var/mob/living/carbon/M in ohearers(6, chassis))
			...
			if(!M.sleeping_willingly)
				M.sleeping = 0
			M.stuttering += 20 * getupgrade("mech_honk_loudness")
			M.ear_deaf += 30 * getupgrade("mech_honk_loudness")
			M.Weaken(3)
			if(prob(30 * getupgrade("mech_honk_loudness")))
				M.Stun(10)
				M.Paralyse(4)
			else
				M.make_jittery(500)
				...
				if(isobj(H.shoes))
					...
					spawn(20)
						if(thingy)
							walk(thingy,0)
			*/
		chassis.use_power(energy_drain / (getupgrade("mech_honk_efficiency") * getupgrade("mech_weapon_efficiency")))
	...
	proc/rearm()
		if(projectiles < initial(projectiles))
			...
			while(chassis.get_charge() >= projectile_energy_cost && projectiles_to_add)
				projectiles++
				projectiles_to_add--
				chassis.use_power(projectile_energy_cost / getupgrade("mech_weapon_efficiency"))
