...
// TODO DOOHL: Intergrate params to new proc. Saved for another time because var/valid_place is a fucking brainfuck
	...
	if(!istype(usr, /mob/living/silicon/ai))
		if (usr.frozen || usr.paralysis || usr.stunned || usr.weakened)
			return
...
//		world << "atom.DblClick() on [src] by [usr] : src.type is [src.type]"
	...
	if (((usr.frozen || usr.paralysis || usr.stunned || usr.weakened) && !istype(usr, /mob/living/silicon/ai)) || usr.stat != 0)
		return
...
// Rotate any machine. For use with stuff like recalculating indir and stuff like that.
/atom/proc/setdirection(var/newdir)
	dir = newdir
