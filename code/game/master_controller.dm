...
datum/controller/game_controller
	...
	setup()
		...
		if(!puddle_controller)
			puddle_controller = new /datum/liquid_controller()

		if(!liquidnet_controller)
			liquidnet_controller = new /datum/liquidnet()

		if(!upgrade_controller)
			upgrade_controller = new /datum/upgradecontrol()
...
// Because energy cutlasses, facehuggers, and empty rooms are silly. FOR NOW. - Erthilo
	...
	process()
		...
		src.set_debug_state("Splash Processing")
		if(puddle_controller)
			if(!puddle_controller.processing)
				puddle_controller.process()
		if(liquidnet_controller)
			if(!liquidnet_controller.processing)
				liquidnet_controller.process()
		puddles_ready = 1
