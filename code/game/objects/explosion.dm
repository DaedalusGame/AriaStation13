...
proc/oldexplosion(turf/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, adminlog = 1)
	if(!epicenter) return
...
proc/explosioncirclerangeturfs(center=usr,radius=3)
...
proc/explosion(turf/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, adminlog = 1)
	...
	spawn(0)
		...
		var/list/aTurfs = list()	//Holds the turfs in devestation range.
		//var/list/hTurfs = list()	//Holds the turfs in heavy impact range, minus turfs in devestation range.
		//var/list/lTurfs = list()	//Holds the turfs in light impact range, minus turfs in devestation range and heavy impact range.
		//var/list/fTurfs = list()	//Holds turfs to loop through for mobs to flash. (Hehehe, dirty)
		...
		if(roundExplosions)
			if(locate(/obj/effect/energy_field) in range(src, max(devastation_range, heavy_impact_range, light_impact_range)))
				aTurfs = circlerangeturfs(epicenter,max(devastation_range, heavy_impact_range, light_impact_range, flash_range))
				//dTurfs = circlerangeturfs(epicenter,devastation_range)
				//hTurfs = circlerangeturfs(epicenter,heavy_impact_range) - dTurfs
				//lTurfs = circlerangeturfs(epicenter,light_impact_range) - dTurfs - hTurfs
			else
			...
			if(/obj/effect/energy_field in range(src, max(devastation_range, heavy_impact_range, light_impact_range)))
				aTurfs = range(epicenter,max(devastation_range, heavy_impact_range, light_impact_range, flash_range))
				//dTurfs = range(epicenter,devastation_range)
				//hTurfs = range(epicenter,heavy_impact_range) - dTurfs
				//lTurfs = range(epicenter,light_impact_range) - dTurfs - hTurfs
			else
		...
		var/lrange = light_impact_range*light_impact_range
		var/hrange = heavy_impact_range*heavy_impact_range
		var/drange = devastation_range*devastation_range
		var/frange = flash_range*flash_range

		if(light_impact_range == -1) lrange = -1
		if(heavy_impact_range == -1) hrange = -1
		if(devastation_range == -1) drange = -1
		if(flash_range == -1) frange = -1


		for(var/turf/T in aTurfs) //Loop through the turfs in devestation range.
			//spawn() //Try to pop each turf into it's own thread, speed things along. //FUCKING STOP -BORD
			if(T) //Sanity checking.
				//Now, the actual explosion stuff happens.
				var/d = getsqrdist(T,epicenter)

				for(var/atom/object in T.contents)
					//spawn() //WHAT IS WRONG WITH YOU PEOPLE
					if(object)
						if(istype(object,/mob/living/carbon) && d <= frange)
							var/mob/living/carbon/mob = object
							flick("flash", mob:flash)
						if(object && d <= drange)
							object.ex_act(1)
						if(object && d <= hrange)
...
proc/getsqrdist(atom/a,atom/b)
	var/distx = abs(a.x - b.x)
	var/disty = abs(a.y - b.y)
