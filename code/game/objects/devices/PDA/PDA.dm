...
//MESSENGER/NOTE FUNCTIONS===================================
				...
				if("Message")
					...
					if(!istype(P) || is_bluespaced(P))	return
					...
					if(istype(P, /obj/item/device/pda))
						...
						if(message_servers && !is_bluespaced(src))
...
//SYNDICATE FUNCTIONS===================================
				...
				if("Toggle Door")
					if(!isnull(cartridge) && !is_bluespaced(src) && cartridge.access_remote_door)
...
/mob/living/silicon/ai/proc/ai_send_pdamesg(obj/selected as obj)
	...
	if (selected:toff)
		return
	var/obj/machinery/message_server/useMS = null
	if(!isnull(message_servers) && !is_bluespaced(src))
		for (var/obj/machinery/message_server/MS in message_servers)
		//PDAs are now dependant on the Message Server.
			if(MS.active)
				useMS = MS
