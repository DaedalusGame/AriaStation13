...
/obj/item/weapon/weldingtool/process()
	switch(welding)
		//If off
		if(0)
			if(src.icon_state != "[base_state]") //Check that the sprite is correct, if it isnt, it means toggle() was not called
				src.force = 3
				src.damtype = "brute"
		...
		if(1)
			if(src.icon_state != "[base_state]1") //Check that the sprite is correct, if it isnt, it means toggle() was not called
				src.force = 15
				src.damtype = "fire"
...
/obj/item/weapon/weldingtool/afterattack(obj/O as obj, mob/user as mob)
	if (istype(O, /obj/structure/reagent_dispensers/fueltank) && get_dist(src,O) <= 1 && !src.welding)
		if(O.reagents.trans_to(src, max_fuel))
...
/obj/item/weapon/weldingtool/arc
	...
	base_state = "arcwelder"
	var/obj/item/weapon/cell/battery = null

/obj/item/weapon/weldingtool/arc/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W,/obj/item/weapon/cell))
		if(welding)
			user << "\red Stop welding first!"
			return
		if(!battery)
			user << "\blue You insert the [W] into \the [src]."
			usr.before_take_item(W)
			W.loc = src
			battery = W
			updateicon()
		else
			user << "\blue The welder already contains a powercell."
		src.add_fingerprint(user)
		return

	..()

/obj/item/weapon/weldingtool/arc/attack_self(mob/user as mob)
	if(get_fuel() <= 0 && !welding && battery)
		user << "\blue You eject the empty [battery] from \the [src]."
		battery.loc = src.loc
		battery = null
		updateicon()
		return
...
/obj/item/weapon/weldingtool/arc/proc/updateicon()
	overlays = list()
	if(battery)
		var/fuellevel = battery.percent()
		switch (fuellevel)
			if (0 to 10)
				overlays += "arcwelder+0"
			if (10 to 50)
				overlays += "arcwelder+1"
			if (50 to 90)
				overlays += "arcwelder+2"
			if (90 to 100)
				overlays += "arcwelder+3"

/obj/item/weapon/weldingtool/arc/process()
	..()
	updateicon()

/obj/item/weapon/weldingtool/arc/get_fuel()
	if(!battery)
		return 0
...
/obj/item/weapon/weldingtool/arc/remove_fuel(var/amount = 1, var/mob/M = null)
	...
	if(get_fuel() >= amount)
		...
		if(M)
			M << "\blue You need more charge to complete this task."
		return 0

/obj/item/weapon/weldingtool/nuclear
	name = "Nuclear Welding Tool"
	max_fuel = 40
	w_class = 3.0
	m_amt = 70
	g_amt = 120
	origin_tech = "engineering=4;material=6"
	icon_state = "nukewelder"
	base_state = "nukewelder"
	var/last_gen = 0
	var/lightfail = 0

/obj/item/weapon/weldingtool/nuclear/process()
	..()
	if(crit_fail)
		return
	if(failcheck())
	...
	if (prob(src.reliability))
		for (var/mob/living/M in range(0,src)) //Only a minor failure, enjoy your radiation if you're in the same tile or carrying it
			if (src in M.contents)
				M << "\red Your welder feels pleasantly warm for a moment."
			else
		...
		for (var/mob/living/M in range(rand(1,4),src)) //Big failure, TIME FOR RADIATION BITCHES
			if (src in M.contents)
				M << "\red Your welder's reactor overloads!"
...
/obj/item/weapon/weldingtool/nuclear/emp_act(severity)
	..()
