				...
				if(istype(M, /mob/living/carbon/metroid))
					if(M.stat == 2)
						...
						if(Metroid.cores > 0)
							...
							Metroid.cores--
							M << "\red [user] begins to remove one of your cores with [src]! ([Metroid.cores + Metroid.phazoncores] cores remaining)"
							user << "\red You cut one of [M]'s cores out with [src]! ([Metroid.cores + Metroid.phazoncores] cores remaining)"

							new/obj/item/metroid_core(M.loc)

							if(Metroid.cores && Metroid.phazoncores <= 0)
								M.icon_state = "baby metroid dead-nocore"

						if(Metroid.phazoncores > 0)
							for(var/mob/O in (viewers(M) - user - M))
								O.show_message("\red [M.name] is having one of its cores sawed out with [src] by [user].", 1)
							...
							new/obj/item/metroid_core/phazon(M.loc)
