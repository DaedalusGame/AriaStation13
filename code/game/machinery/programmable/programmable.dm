...
/obj/machinery/programmable
	...
	var/list/overrides = list(new/datum/cargoprofile/cargo/unload(),new/datum/cargoprofile/in_stacker(),new/datum/cargoprofile/seedboxer())
...
/obj/machinery/programmable/proc/interact(mob/user as mob)
	var/dat
...
/obj/machinery/programmable/loader
	name = "Massdriver Loader"
	default = new/datum/cargoprofile/massdriver()
	profiles = list()
	overrides = list()
	emag_overrides = list()

/obj/machinery/programmable/wakemeupinside
	name = "\"Providence\"-class Recovery Unit"
	default = new/datum/cargoprofile/people()
	profiles = list()
	overrides = list()
	emag_overrides = list()
...
//      computations every round needlessly if you add too many profiles
//----------------------------------------------------------------------------
/obj/machinery/programmable/unary
	name = "Programmable Processor"
	default = null
	profiles = list()
	overrides = list(new/datum/cargoprofile/unary/stacker())
	...
	interact(mob/user as mob)
		...
		if(overrides.len)
			...
			dat	+= printlist(overrides)
		dat += "</TT>"
		user << browse("<HEAD><TITLE>Processor</TITLE></HEAD>[dat]", "window=progreload")
		onclose(user, "progreload")
		return
...
/obj/machinery/programmable/unary/trainer
	...
	attack_hand(mob/user as mob)
		...
		//How did I type this with boxing gloves on?
		...
		if(H.gloves && istype(H.gloves, /obj/item/clothing/gloves/boxing))
			...
			if(SUPRSTR in H.augmentations)	damage += 5
			...
			while(damage >= 5)
				if(prob(50))
					newsleep++
				damage -= 5
			if(newsleep && !sleep) // no message if it's already sleeping
				if(emagged)
...
/obj/machinery/programmable/bidir
	...
	overrides = list()
	emag_overrides = list()
	var/list/reverse_profiles = list()
	var/list/reverse_overrides = list()
	var/list/reverse_emag_overrides = list()
*/
	//To be overridden:
	// new
	// interact
...
/obj/item/weapon/circuitboard/programmable
	name = "Circuit board (Selective Unloader)"
	...
	proc/resetlists()
		...
		overrides = list(new/datum/cargoprofile/cargo/unload(),new/datum/cargoprofile/in_stacker(),new/datum/cargoprofile/seedboxer(),
