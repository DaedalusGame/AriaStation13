...
/obj/machinery/bot/mulebot/proc/interact(var/mob/user, var/ai=0)
	...
	if(!open)
		if(ai)
			dat += "<HR><A href='byond://?src=\ref[src];op=remote'><I>[remoting ? "End Remoting" : "Begin Remoting"]</I></A><BR><BR>"
...
/obj/machinery/bot/mulebot/Topic(href, href_list)
	...
	if ((in_range(src, usr) && istype(src.loc, /turf)) || (istype(usr, /mob/living/silicon)))
		...
		switch(href_list["op"])
			...
			if("remote")
				if(remoting)
					remoting.unsetremote()
				else
					var/mob/living/silicon/ai/remoter = usr
					if(istype(remoter))
						remoter.setremote(src)
...
/obj/machinery/bot/mulebot/remotemove(var/direct)
	if(!on || !remoting)
		return

	var/turf/next = get_step(src,direct)
...
/obj/machinery/bot/mulebot/Bump(var/atom/obs)
	if(!(wires & wire_mobavoid))		//usually just bumps, but if avoidance disabled knock over mobs
		...
		if(istype(obs,/atom/movable))
			var/atom/movable/LC = obs
			if(!(LC.anchored || load))
				load(LC)
	..()
