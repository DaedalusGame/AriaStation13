...
/obj/machinery/gibber/proc/startgibbing(mob/user as mob)
	...
	if(istype(occupant,/mob/living/carbon/human))
		...
		for (var/i=1 to totalslabs)
			if(occupant.frozen)
				var/icetype = pick(/obj/item/weapon/reagent_containers/food/snacks/icecream,/obj/item/weapon/reagent_containers/food/snacks/bloodcream)
				var/obj/item/weapon/reagent_containers/food/snacks/ice = new icetype()
				ice.name = sourcename + ice.name
				src.occupant.reagents.trans_to(ice, round(sourcetotalreagents / totalslabs, 1))
				allmeat += ice
			else
		...
		for (var/i=1 to allmeat.len)
			...
			if (!Tx.density)
				switch(gibrace)
					...
					if("alien")
						new /obj/effect/decal/cleanable/xenoblood/xgibs(Tx,i + iterator)
					if("ice")
						new /obj/effect/decal/cleanable/blood/gibs/frost(Tx,i + iterator)
		src.operating = 0
		update_icon()
