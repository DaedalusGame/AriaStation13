...
/obj/item/pipe/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	..()
	//*
	if(istype(W,/obj/item/weapon/weldingtool) && (pipe_type in list(PIPE_MVALVE,PIPE_MANIFOLD)))
		var/obj/item/weapon/weldingtool/WT = W
		if( WT.remove_fuel(0,user) )
			user << "<span class='notice'>You slice the pipe apart.</span>"
			playsound(src.loc, 'Welder.ogg', 100, 1)
			if(pipe_type == PIPE_MANIFOLD)
				new /obj/item/pipeassembly(src.loc)
			else
				new /obj/item/pipeassembly/valve(src.loc)
				new /obj/item/pipeassembly/endpiece(src.loc)
				new /obj/item/pipeassembly/endpiece(src.loc)
			del(src)
			return
		else
