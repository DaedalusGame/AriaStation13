...
/datum/game_mode/proc/generate_crew_objectives()
	for(var/datum/mind/crewmember in crew)
		forge_crew_objectives(crewmember)
		greet_crewmember(crewmember)


/datum/game_mode/proc/forge_crew_objectives(var/datum/mind/crewmember)
	if(!istype(crewmember.current, /mob/living/silicon))
		if(!crewmember.objectives || crewmember.objectives.len)
			return

		for(var/datum/objective/o in SelectCrewObjectives((istype(crewmember.current:wear_id, /obj/item/weapon/card/id) ? crewmember.current:wear_id:assignment : crewmember.assigned_role), crewmember))
			o.owner = crewmember
			crewmember.objectives += o
	return

/datum/game_mode/proc/greet_crewmember(var/datum/mind/crewmember)
	crewmember.current << "<B>You are an employee under Nanotransen, you are to do your job as you were instructed how to. You have some optional objectives, they are as follows.</B>"
	var/obj_count = 1
	for(var/datum/objective/objective in crewmember.objectives)
		crewmember.current << "<B>Objective #[obj_count]</B>: [objective.explanation_text]"
		obj_count++
	return
...
/datum/game_mode/proc/latespawn(var/mob)
	...
	var/mob/living/carbon/human/character = mob

	if(!istype(character))
		return

	if(character.client)
		forge_crew_objectives(character.mind)
		greet_crewmember(character.mind)
