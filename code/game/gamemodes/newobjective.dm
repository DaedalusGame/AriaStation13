...
/proc/GenerateJob(var/job,var/datum/mind/traitor)
	var/list/datum/objective/objectives = list()

	for(var/o in typesof(/datum/objective/crew))
		if(o != /datum/objective/crew)		//Make sure not to get a blank steal objective.
...
/proc/SelectCrewObjectives(var/job,var/datum/mind/crewmember)
	var/list/chosenobjectives = list()
	var/list/crewobjectives = GenerateJob(job,crewmember)
	var/total_weight
	var/target_weight
	...
	while(total_weight < target_weight)
		var/datum/objective/objective = pickweight(crewobjectives)
		if(objective)
			chosenobjectives += objective
			total_weight += objective.points
			crewobjectives -= objective
		else
			break
