...
/mob/living/verb/resist()
	...
	if (usr.lying && !( usr.stat ) && usr.burning)
		usr << "\red You roll around!"

		for(var/mob/O in viewers(usr, null))
			O.show_message(text("\red [] is struggling against the fire!", usr), 1)
		...
		spawn(5)
			if(usr.burnlevel <= 0)
				for(var/mob/O in viewers(usr, null))
					O.show_message(text("\green [] has managed to put the fire out!", usr), 1)
				usr << "\green You successfully put out the fire!"
	if ((!( usr.stat ) && usr.canmove && !( usr.restrained() )))
		var/resisting = 0
