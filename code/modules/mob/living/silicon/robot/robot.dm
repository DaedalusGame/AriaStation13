...
/mob/living/silicon/robot/proc/pick_module()
	...
	switch(mod)
		...
		if("Service")
			...
			var/icontype = input("Select an icon!", "Robot", null, null) in list("Waitress", "Bro", "Butler", "Kent", "Rich", "Corgi", "Maid")
			...
			else if(icontype == "Corgi")
				icon_state = "corgi"
			else if(icontype == "Maid")
				icon_state = "maidbot"
			else
				icon_state = "Service2"
			modtype = "Butler"
		...
		if("Security")
			var/sprite = input(src,"Chassis Style", "Chassis Style", "Cancel") in list("Humanoid","Bloodhound","Enforcer")
			updatename(mod)
			module = new /obj/item/weapon/robot_module/security(src)
			hands.icon_state = "security"
			if (sprite == "Humanoid")
				src.icon_state = "securityrobot"
			if (sprite == "Bloodhound")
				src.icon_state = "bloodhound"
			if (sprite == "Enforcer")
				src.icon_state = "enforcer"
			modtype = "Sec"
		...
		if("Engineering")
			updatename(mod)
			var/sprite = input(src,"Chassis Style", "Chassis Style", "Cancel") in list("Humanoid","Landmate","DS-Tech")
