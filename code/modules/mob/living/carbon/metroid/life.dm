...
/mob/living/carbon/metroid
	...
	proc
		...
		AIprocess()  // the master AI process
			...
			if(AIproc || frozen || stat == 2 || client) return
		...
		handle_environment(datum/gas_mixture/environment)
			if(!environment && !COLD_RESISTANCE in mutations)
			...
			*/
			...
			if(!COLD_RESISTANCE in mutations && bodytemperature < (T0C + 5)) // start calculating temperature damage etc
			...
			if(bodytemperature <= (T0C - 40) || bodytemperature >= (T0C + 30))
				freezelevel += 40.0*(T0C - bodytemperature)/T0C //Falling to 100 K will freeze instantly
			...
			if(freezelevel >= 10)
				...
				Tempstun = 1
				canmove = 0
			if(freezelevel <= 0 || COLD_RESISTANCE in mutations)
				freezelevel = 0
				frozen = 0
				Tempstun = 0
				canmove = 1
			...
			if(frozen != oldfrozen)
				if(frozen)
					overlays += image("icon" = 'mob.dmi', "icon_state" = "frozen_s")
				else
					overlays = null
