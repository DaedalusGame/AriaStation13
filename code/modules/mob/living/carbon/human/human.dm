...
/*		if("headset")
		...
		if("o_clothing")
			...
			if (!( W.slot_flags & SLOT_OCLOTHING ))
				return
			if (happyfuta)
				src << "You have a problem dressing up. A HUGE problem."
				return
			u_equip(W)
			wear_suit = W
			W.equipped(src, text)
...
/mob/living/carbon/human/proc/rebuild_body_overlays()
	...
	if(frozen)
		body_overlays_standing += image("icon" = 'mob.dmi', "icon_state" = "frozen_s")
		body_overlays_lying	   += image("icon" = 'mob.dmi', "icon_state" = "frozen_l")

	if(burning)
		body_overlays_standing += image("icon" = 'mob.dmi', "icon_state" = "fire_s")
		body_overlays_lying	   += image("icon" = 'mob.dmi', "icon_state" = "fire_l")
...
/mob/living/carbon/human/proc/rebuild_clothing_overlays()
	...
	if (FUTA in mutations)
		//src << "Calling futa updates..."

		if(!istype(wear_suit,/obj/item/clothing/suit) && !istype(w_uniform,/obj/item/clothing/under))
			//src << "You're naked."

			var/icon/penis = icon('human.dmi', "penis[happyfuta]_[lying?"l":"s"]")

			if(s_tone >= 0)
				penis.Blend(rgb(s_tone, s_tone, s_tone), ICON_ADD)
			else
				penis.Blend(rgb(-s_tone,  -s_tone,  -s_tone), ICON_SUBTRACT)
			if(pale)
				penis.Blend(rgb(100,100,100))

			clothing_overlays += penis
