...
/mob/living/carbon/human
	proc
		...
		handle_health_updates()
			...
			//Duplicates so dead people can still be extinguishes or thawed.
			if(freezelevel >= 10)
				freezelevel = 10
				frozen = 1
			else if(freezelevel < 0)
				freezelevel = 0
				frozen = 0
		...
		handle_mutations_and_radiation()
			...
			if (FUTA in mutations)
				var/horny = 0

				for(var/mob/living/carbon/human/H in oview(5,src))
					if(H.bruteloss > 10 || H.fireloss > 10 || H.oxyloss > 10 || H.toxloss > 10 || H.cloneloss > 0 || H.brainloss > 0 || H.stat == 2)
						horny -= 1
					else
						horny += 10

				if(bruteloss > 20 || fireloss > 20 || oxyloss > 40 || toxloss > 30 || cloneloss > 0 || brainloss > 0 || stat != 0)
					horny -= 10

				if(horny == 0)
					horny = -0.002

				hornyfuta = max(0,min(20,hornyfuta + horny))

				if(hornyfuta >= 20 && !happyfuta)
					src << "You suddenly feel like you're going to have a serious problem..."
					happyfuta = 1
					rebuild_appearance()
				else if(hornyfuta <= 0 && happyfuta)
					happyfuta = 0
					rebuild_appearance()
		...
		**/
		...
		update_canmove()
			if(stat || frozen || sleeping || paralysis || stunned || weakened || resting || buckled || (changeling && changeling.changeling_fakedeath))
				canmove = 0
		...
		handle_breath(datum/gas_mixture/breath)
			...
			if(breath.trace_gases.len)	// If there's some other shit in the air lets deal with it here.
				...
				for(var/datum/gas/reagent/LA in breath.trace_gases)
					LA.liquid.reaction_mob(src, INGEST, 100*LA.moles)
					reagents.add_reagent(LA.liquid.id, 100*LA.moles)
					LA.moles = 0
		...
		handle_environment(datum/gas_mixture/environment)
			...
			//if(frozen)
				//bodytemperature += adjust_body_temperature(bodytemperature, T0C, 1/thermal_protection)
			//if(burning)
				//bodytemperature += adjust_body_temperature(bodytemperature, T0C+1000, 1/thermal_protection)
			...
			*/
			...
			var/oldfrozen = frozen
			var/oldburning = burning
			...
			if(COLD_RESISTANCE in mutations)
				...
				freezelevel += (frozen ? 1.0 : 20.0)*(T0C - bodytemperature)/T0C //Falling to 100 K will freeze instantly

				if(freezelevel >= 10)
					freezelevel = 10
					frozen = 1
				else if(freezelevel < 0)// || COLD_RESISTANCE in mutations)
					freezelevel = 0
					frozen = 0

				burnlevel += (burning ? 1.0 : 20.0)*(bodytemperature - (T0C + 100))/T0C

				if(burnlevel >= 10)
					burnlevel = 10
					burning = 1
				else if(burnlevel < 0)// || COLD_RESISTANCE in mutations)
					burnlevel = 0
					burning = 0

			if(burning != oldburning || frozen != oldfrozen)
				update_body_appearance()

			if(burning)
				FireBurn(1)
				//adjustFireLoss((burnlevel / 5.0) * (bodytemperature - (T0C + 100)) / (T0C + 100),min(10))
			...
			if(environment.trace_gases.len)	// If there's some other shit in the air lets deal with it here.
				var/blockchance = 0

				if(head)
					if(head.flags & HEADCOVERSEYES) blockchance += 50

				if(wear_suit)
					if(wear_suit.flags_inv & HIDEJUMPSUIT) blockchance += 50

				for(var/datum/gas/reagent/LA in environment.trace_gases)
					if(!prob(blockchance))
						LA.liquid.reaction_mob(src, TOUCH, 100*LA.moles)
...
#define healthcolor_0 "#009900"
#define healthcolor_1 "#009900"
#define healthcolor_2 "#995500"
#define healthcolor_3 "#999900"
#define healthcolor_4 "#cc6600"
#define healthcolor_5 "#cc0000"

		update_healthicon()
			healths.overlays.Cut()

			var/overallhealth = health - halloss
			...
			for(var/named in organs)
				...
				var/organhealth = ((F.min_broken_damage - F.get_damage()) / F.min_broken_damage) * 100

				//world << "organ [F.name]; health = [organhealth]"

				if(F.status & ORGAN_DESTROYED)
					continue

				healthicon = icon(healths.icon, F.icon_name)
				healthicon.Blend(get_healthcolor(organhealth))
				healths.overlays += healthicon

			healthicon = icon(healths.icon, "overall")
			healthicon.Blend(get_healthcolor(overallhealth))
			healths.overlays += healthicon

				// if he's in crit, display crit icon
			if (stat == 2 || hal_screwyhud == 2)
				healths.overlays += icon(healths.icon, "dead")
			else if(overallhealth < config.health_threshold_crit || hal_screwyhud == 1)
				healths.overlays += icon(healths.icon, "crit")

		get_healthcolor(var/val)
			switch(val)
				if(100 to INFINITY)
					return healthcolor_0
				if(80 to 100)
					return healthcolor_1
				if(60 to 80)
					return healthcolor_2
				if(40 to 60)
					return healthcolor_3
				if(20 to 40)
					return healthcolor_4
				else
					return healthcolor_5
