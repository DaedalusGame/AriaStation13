...
/proc/shake_camera(mob/M, duration, strength=1)
	...
	spawn(1)
		...
		for(x=0; x<duration, x++)
			//animate(M.client,pixel_x = rand(-strength*8,strength*8),pixel_y = rand(-strength*8,strength*8), time = 2)
			//for(y=0; y<(world.fps / 2) * 10, y++)
			//M.client.eye = locate(dd_range(1,M.loc.x+rand(-strength,strength),world.maxx),dd_range(1,M.loc.y+rand(-strength,strength),world.maxy),M.loc.z)
			//	animate(pixel_x = rand(-strength*8,strength*8),pixel_y = rand(-strength*8,strength*8), time = 2)
			M.client.pixel_x = rand(-strength*8,strength*8)
			M.client.pixel_y = rand(-strength*8,strength*8)
			sleep(0.5)
		//animate(M.client)
