		...
		if((brute_dam + burn_dam + brute + burn) < max_damage)
			...
			if(can_inflict)
				...
				if(!possible_points.len)
					//message_admins("Oh god WHAT!  [owner]'s [src] was unable to find an organ to pass overdamage too!") Fixed that shitty spam.
				else
			...
			switch(body_part)
				...
				if(LOWER_TORSO)
					for(var/datum/disease/D in owner.viruses)
						if(istype(D,/datum/disease/futa_transformation))
							D.cure()
