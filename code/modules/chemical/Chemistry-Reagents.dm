	...
	reagent
		...
		proc
			...
			reaction_turf(var/turf/T, var/volume)
				...
				var/obj/floorchem/C = locate(/obj/floorchem) in T

				if(!C)
					C = new /obj/floorchem(T, volume)
		...
		metroid
			...
			reaction_mob(var/mob/M, var/method=TOUCH, var/volume)
				if(istype(M,/mob/living/carbon/metroid) && method == INGEST)
					M.adjustFireLoss(-5 * volume)
					M.setToxLoss(0)
					M.SetWeakened(0)
					M.SetStunned(0)
					M.SetParalysis(0)
...
*/
		...
		unimatter
			...
			id = "unimatter"
			reagent_state = LIQUID
			color = "#C810C8" // rgb: 200, 16, 200


			//This stuff has a random effect whenever it's splashed or metabolized and then annihilates part of itself
			reaction_mob(var/mob/M, var/method=TOUCH, var/volume)
				var/datum/reagent/random = chemical_reagents_list[pick(chemical_reagents_list)]

				if(random)
					random.reaction_mob(M,method,volume)

			reaction_obj(var/obj/O, var/volume)
				var/datum/reagent/random = chemical_reagents_list[pick(chemical_reagents_list)]

				if(random)
					random.reaction_obj(O,volume)

			reaction_turf(var/turf/simulated/T, var/volume)
				var/datum/reagent/random = chemical_reagents_list[pick(chemical_reagents_list)]

				if(random)
					random.reaction_turf(T,volume)
			...
			on_mob_life(var/mob/living/M as mob)
				...
				holder.add_reagent(random.id, REAGENTS_METABOLISM)
				return
		...
		lube
			...
			reaction_turf(var/turf/simulated/T, var/volume)
				...
				if(T.wet >= 2) return
				T.wet = 2
				T.wet_overlay = image('water.dmi',T,"gel")
				T.overlays += T.wet_overlay
				spawn(800)
					if (!istype(T)) return
					T.wet = 0
...
//					M:adjustToxLoss(0.2)
		...
		phazon
			name = "Phazon"
			id = "phazon"
			description = "Highly cancerogen and toxic material. Do not contact, inhale or ingest!"
			reagent_state = LIQUID
			color = "#13BCBC" // rgb: 19, 188, 94

			reaction_mob(var/mob/M, var/method=TOUCH, var/volume)
				if(!..())	return
				if(isrobot(M) || isAI(M)) return // Phazon doesn't do anything to robutts!
				src = null
				if((method==TOUCH && prob(66)) || method==INGEST)
					randmuti(M)
					if(prob(60))
						randmutb(M)
					else
			...
			reaction_obj(var/obj/O, var/volume)
				...
				if(istype(O,/obj/item/weapon/reagent_containers/food/snacks))
					for(var/mob/M in viewers(5, O))
						M << "\red \the [O] withers away."
					var/obj/item/weapon/reagent_containers/food/snacks/badrecipe/b = new(O.loc)
					b.name = "withered matter"
					b.desc = "This looks highly toxic."
					del(O)
				return
			...
			reaction_turf(var/turf/T, var/volume)
				src = null
				if(prob(volume))
					new /obj/effect/phazon(T)
				return

			on_mob_life(var/mob/living/M as mob)
				if(isrobot(M) || isAI(M)) return // Mutagen doesn't do anything to robutts!
				if(!M) M = holder.my_atom
				if(M.stat == 2 && isturf(M.loc)) reaction_turf(M.loc,volume)
				M.radiation += 5 //KILLS STUFF DEAD.
				..()
				return
		...
		lava
			name = "Lava"
			id = "lava"
			description = "A molten glob of liquid mineral."
			reagent_state = LIQUID
...
/*			reaction_obj(var/obj/O, var/volume)
	...
	*/
		...
		carbonfiber
			name = "Carbonfiber"
			id = "coalfiber"
			description = "Fibric material made from carbon."
			reagent_state = SOLID
			color = "#030303"
		...
		rubber
			name = "Rubber"
			id = "rubber"
			description = "An elastic material."
			reagent_state = LIQUID
			color = "#FCA8FC"
		...
		latex
			name = "Latex"
			id = "latex"
			description = "A refined form of rubber."
			reagent_state = LIQUID
			color = "#A8A4A8"
		...
		iron
			name = "Iron"
			id = "iron"
			description = "Pure iron is a metal." //You don't say.
			reagent_state = SOLID
			color = "#C8A5DC" // rgb: 200, 165, 220
/*
			on_mob_life(var/mob/living/M as mob)
...
*/
		...
		glass
			name = "Glass"
			id = "glass"
			description = "A transparent material made from heating sand"
			reagent_state = SOLID
			color = "#D0D0D0"
		...
		adamantine
			name = "Adamantine"
			id = "adamantine"
			description = "A material extracted from Wolverine's bones."
			reagent_state = SOLID
			color = "#A8A8A8"
		...
		diamond
			name = "Diamond"
			id = "diamond"
			description = "A superhard form of carbon in powderform."
			reagent_state = SOLID
			color = "#FAFAFF"
		...
		liquidnitrogen
			...
			on_mob_life(var/mob/living/M as mob)
				...
				if(prob(5)) // leaves someone at 100% healthy from anything up to in heavy crit (-75%)
					M.take_organ_damage(0, 1)
				if(prob(80) && istype(M, /mob/living/carbon/metroid) && !COLD_RESISTANCE in M.mutations)
