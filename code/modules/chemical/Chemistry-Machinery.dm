...
/obj/machinery/chem_dispenser/
	...
	var/list/blacklist = list("unimatter","lava")
	...
	proc/dispense(id,amt)
		if(stat & NOPOWER) return
		if(id in blacklist) return
		...
		if (dispensable_reagents.Find(id) && beaker != null)
			var/obj/item/weapon/reagent_containers/glass/B = src.beaker
			R.trans_id_to(B,id,amt)
	...
	SkinCmd(mob/user as mob, var/data as text)
		...
		amount = round(amount, 1) // Chem dispenser doesnt really have that much prescion
...
/obj/machinery/chem_dispenser/process()
	...
	for(var/datum/reagent/chem in R.reagent_list)
		if(chem.id in blacklist) continue

		dispensable_reagents += chem.id
...
/obj/machinery/reagentgrinder
	...
	var/global/list/allowed_items = list ( // reagent = amount, amount of 0 indicate to determine the amount from the reagents list, only implemented on plants for now
		...
		*/
		...
		/obj/item/weapon/ore/coal = list("carbon" = 20),
		/obj/item/weapon/ore/phazon = list("phazon" = 20),
	)
