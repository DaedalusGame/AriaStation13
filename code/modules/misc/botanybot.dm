//Botanybot
/obj/machinery/bot/botanybot
	name = "Botanybot"
	desc = "A heavy multi-functional aeroponics robot"
	icon = 'aibots.dmi'
	icon_state = "farmbot0"
	layer = 5.0
	density = 1
	...
	var/panelopen = 0
	var/working = 0
	var/obj/machinery/hydroponics/target
	var/obj/machinery/hydroponics/oldtarget
	var/oldloc = null
...
/obj/machinery/bot/botanybot/New()
	..()
	//src.get_targets()
	...
	src.botcard.access = get_access("Botanist")

	if(radio_controller)
		radio_controller.add_object(src, beacon_freq, filter = RADIO_NAVBEACONS)


/obj/machinery/bot/botanybot/turn_on()
	. = ..()
	src.icon_state = "farmbot[src.on]"
	src.updateUsrDialog()

/obj/machinery/bot/botanybot/turn_off()
	..()
	src.target = null
	src.oldtarget = null
	src.oldloc = null
...
/obj/machinery/bot/botanybot/attack_hand(mob/user as mob)
	. = ..()
	if (.)
		return
	usr.machine = src
...
/obj/machinery/bot/botanybot/proc/interact(mob/user as mob)
	var/dat
	dat += text({"
...
text("<A href='?src=\ref[src];operation=oddbutton'>[src.oddbutton ? "Yes" : "No"]</A>"))*/
	...
	user << browse("<HEAD><TITLE>Farmer v0.6.1 controls</TITLE></HEAD>[dat]", "window=autofarmer")
	onclose(user, "autofarmer")
	return

/obj/machinery/bot/botanybot/Topic(href, href_list)
	if(..())
		return
	usr.machine = src
...
/obj/machinery/bot/botanybot/attackby(obj/item/weapon/W, mob/user as mob)
...
/obj/machinery/bot/botanybot/Emag(mob/user as mob)
	..()
...
/obj/machinery/bot/botanybot/process()
	set background = 1
	...
	if(src.working)
		return
	var/list/farmbottargets = list()
	if(!src.target || src.target == null)
		for(var/obj/machinery/bot/botanybot/bot in world)
			if(bot != src)
				farmbottargets += bot.target
	...
	if(!src.target || src.target == null)
		for (var/obj/machinery/hydroponics/Tray in view(7,src))
			if(!(Tray in farmbottargets) && Tray != src.oldtarget)
				src.oldtarget = Tray
				src.target = Tray
				return
...
/obj/machinery/bot/botanybot/proc/patrol_move()
...
/obj/machinery/bot/botanybot/receive_signal(datum/signal/signal)
...
/obj/machinery/bot/botanybot/proc/work(var/obj/machinery/hydroponics/target)
	src.anchored = 1
...
/obj/machinery/bot/botanybot/explode()
	src.on = 0
