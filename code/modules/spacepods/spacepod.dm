...
var/global/list/spacepods_list = list()
...
/obj/spacepod/proc/do_motor()
	if(!src || !motoron) return

	lastmotor = world.time

	speedx = max(-maxspeed,min(maxspeed,speedx))
	speedy = max(-maxspeed,min(maxspeed,speedy))

	var/speed = max(abs(speedx),abs(speedy))
	var/movedir = angle2dir(vector2angle(speedx,speedy))

	if(speed > 10 && lastmove < world.time-(100-speed) / 10)
		handleshipmove(movedir)
...
/obj/spacepod/proc/deal_damage(var/damage)
	...
	if(occupant)
		shake_camera(occupant,1,10)

	if(occupant2)
		shake_camera(occupant2,1,10)

	if(occupant && oldhealth > health && percentage <= 25 && percentage > 0)
		var/sound/S = sound('sound/mecha/critnano.ogg')
		S.wait = 0 //No queue
	...
	if(occupant && oldhealth > health && !health)
		var/sound/S = sound('sound/mecha/internaldmgalarm.ogg')
		S.wait = 0
		S.channel = 0
		S.volume = 50
		occupant << S
...
/obj/spacepod/emp_act(severity)
	switch(severity)
		if(1)
			...
			if(src.occupant2)
				src.occupant2 << "<span class='warning'>The pod console flashes 'Heavy EMP WAVE DETECTED'.</span>" //warn the occupants
	...
	if(src.occupant2)
		if(usr.ckey != src.occupant2.ckey)
			...
			for(var/obj/machinery/door/poddoor/P in oview(3,src))
				if(istype(P, /obj/machinery/door/poddoor))
...
/obj/spacepod/proc/lightsToggle()
	lights = !lights
	if(lights)
		ul_SetLuminosity(luminosity + lights_power)
	else
		ul_SetLuminosity(luminosity - lights_power)
...
/obj/spacepod/Bump(atom/Obstacle)
	..()

	var/dx = abs(x - Obstacle.x)
	var/dy = abs(y - Obstacle.y)
...
/obj/spacepod/proc/handleshipmove(direction)
	var/moveship = 1

	src.dir = direction

	if((direction & 1) && (inertia_dir & 2))
		inertia_dir = 0
		moveship = 0
	if((direction & 2) && (inertia_dir & 1))
		inertia_dir = 0
		moveship = 0
	if((direction & 4) && (inertia_dir & 8))
		inertia_dir = 0
		moveship = 0
	if((direction & 8) && (inertia_dir & 4))
		inertia_dir = 0
		moveship = 0
...
/obj/spacepod/proc/accelerate(xacc,yacc)
	...
	if(abs(xacc) && abs(speedx) >= maxspeed)
		speedy /= 2
	if(abs(yacc) && abs(speedy) >= maxspeed)
		speedx /= 2
