...
/obj/machinery/r_n_d/experimentor/proc/SetTypeReactions()
	...
	for(var/I in typesof(/obj/item))
		...
		if(ispath(I,/obj/item/weapon/stock_parts) || ispath(I,/obj/item/weapon/chem_grenade) || ispath(I,/obj/item/weapon/kitchen))
		...
		if(ispath(I,/obj/item/weapon/rcd) || ispath(I,/obj/item/weapon/chem_grenade) || ispath(I,/obj/item/device/aicard) || ispath(I,/obj/item/weapon/storage/backpack/holding) || /*ispath(I,/obj/item/slime_extract) ||*/ ispath(I,/obj/item/device/bombassembly) || ispath(I,/obj/item/device/transfer_valve))
			var/obj/item/tempCheck = new I()
...
/obj/machinery/r_n_d/experimentor/New()
	...
	component_parts += new /obj/item/weapon/stock_parts/micro_laser(src)
	trackedIan = locate(/mob/living/simple_animal/corgi/Ian) in world
	trackedRuntime = locate(/mob/living/simple_animal/cat/Runtime) in world
	SetTypeReactions()
	RefreshParts()
...
/obj/machinery/r_n_d/experimentor/proc/experiment(var/exp,var/obj/item/exp_on)
	...
	if(exp == SCANTYPE_IRRADIATE)
		...
		if(prob(EFFECT_PROB_MEDIUM-badThingCoeff))
			...
			if(istype(loaded_item,/obj/item/weapon/chem_grenade))
				var/obj/item/weapon/chem_grenade/CG = loaded_item
				spawn(50)
					CG.explode()
			ejectItem()
	////////////////////////////////////////////////////////////////////////////////////////////////
	if(exp == SCANTYPE_GAS)
		...
		if(prob(EFFECT_PROB_LOW) && criticalReaction)
			...
			new /obj/item/stack/sheet/plasma(get_turf(pick(oview(1,src))))
		if(prob(EFFECT_PROB_VERYLOW-badThingCoeff))
	...
	if(exp == SCANTYPE_HEAT)
		...
		if(prob(EFFECT_PROB_VERYLOW-badThingCoeff))
			...
			if(MT)
				...
				var/obj/item/projectile/energy/fireball/FB = new /obj/item/projectile/energy/fireball(start)
				FB.original = MT
				FB.current = start
				FB.yo = MT.y - start.y
				FB.xo = MT.x - start.x
...
/obj/item/weapon/relic/proc/clean(var/mob/user)
	playsound(src.loc, "sparks", rand(25,50), 1)
	var/obj/item/weapon/chem_grenade/cleaner/CL = new/obj/item/weapon/chem_grenade/cleaner(get_turf(user))
	spawn(50)
		CL.explode()
...
/obj/item/weapon/relic/proc/flash(var/mob/user)
	playsound(src.loc, "sparks", rand(25,50), 1)
	var/obj/item/weapon/flashbang/CB = new/obj/item/weapon/flashbang(get_turf(user))
	spawn(50)
		CB.prime()
...
/obj/item/weapon/relic/proc/teleport(var/mob/user)
	...
	spawn(rand(10,30))
		...
		if(src.loc == user) //Because Nuke Ops bringing this back on their shuttle, then looting the ERT area is 2fun4you!
