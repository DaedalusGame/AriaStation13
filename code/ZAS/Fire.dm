...
atom/var/burntemp = INFINITY

//Some legacy definitions so fires can be started.
atom/proc/temperature_expose(datum/gas_mixture/air, exposed_temperature, exposed_volume)
	return null

atom/proc/can_burn(datum/gas_mixture/air, exposed_temperature) //TODO: fuel coating, fuel-filled beakers, etc etc
	return exposed_temperature > burntemp

atom/proc/get_fuellevel()
	return 0
					...
					if(flow)
						...
						if(flow.oxygen > 0.3 && (flow.toxins || fuel || liquid))
							...
							switch(firelevel)
								if(50 to INFINITY)
									icon_state = "t2"
									color = rgb(106,135,183)
									if(LuminosityBlue != 11)
										ul_SetLuminosity(0,9,11)
								if(20 to 50)
									icon_state = "t3"
									color = rgb(235,145,242)
									if(LuminosityBlue != 10)
										ul_SetLuminosity(8,0,10)
								if(6 to 20)
									icon_state = "3"
									color = null
									if(LuminosityRed != 11)
...
obj/liquid_fuel
	...
	can_burn(datum/gas_mixture/air, exposed_temperature)
		return 1
...
/mob/living/carbon/human/proc/FireBurn(mx as num)
	...
	burnlevel += 10*mx*((head_exposure+chest_exposure+groin_exposure+legs_exposure+legs_exposure+arms_exposure+arms_exposure) / 7)
	if(burnlevel >= 10)
		burning = 1
		burnlevel = 10
