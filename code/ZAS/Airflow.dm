...
atom/movable/proc/check_airflow_affected(n)
	return 0

atom/movable/proc/airflow_affect(n)
...
mob/airflow_hit(atom/A)
	...
	Weaken(max(weakened, (istype(A,/obj/item) ? A:w_class : rand(1,5)))) //Heheheh //Y U DO DAT
	. = ..()
...
//		M.show_message("\red <B>[src] slams into [A]!</B>",1,"\red You hear a loud slam!",2)
	...
	if(airflow_speed > 10)
		...
		Stun(max(stunned,paralysis + 3)) //FUKKEN GRIFFN BAY12
	else
		AdjustStunned(round(airflow_speed * vsc.airflow_stun/2))
	. = ..()
