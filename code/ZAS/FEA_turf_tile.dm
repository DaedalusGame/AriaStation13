...
area/var/list/gasoverlays = list()

area/proc/gasOverlay(var/list/gasses)
	if(!src)
		return

	overlays -= gasoverlays

	gasoverlays.Cut()

	for(var/gas in gasses)
		gasoverlays += image('tile_effects.dmi', , gas, FLY_LAYER)
		...
		proc
			...
			/*update_visuals()
				...
				*/
				...
				if(model.toxins > MOLES_PLASMA_VISIBLE)
					gases += "plasma"
				else if(length(model.trace_gases))
					var/datum/gas/sleeping_agent = locate(/datum/gas/sleeping_agent) in model.trace_gases
					var/datum/gas/snow = locate(/datum/gas/snow) in model.trace_gases
					var/datum/gas/rain = locate(/datum/gas/rain) in model.trace_gases
					var/datum/gas/coolant = locate(/datum/gas/coolant) in model.trace_gases

					if(sleeping_agent && (sleeping_agent.moles > 1))
						gases += "sleeping_agent"
				...
				if(!isarea(CurrentArea))
					return*/

				//CurrentArea.gasOverlay(gases)
