...
datum
	...
	gas_mixture
		...
		proc/add_tracegas(var/datum/gas/g,var/amt)
			var/datum/gas/trace_gas = get_tracegas(g)
			if(trace_gas)
				trace_gas.moles += amt
			else
				trace_gas = new g.type()
				if(istype(trace_gas,/datum/gas/reagent))
					var/datum/gas/reagent/T1 = g
					var/datum/gas/reagent/T2 = trace_gas
					T2.liquid = T1.liquid
				trace_gas.moles += amt
				if(trace_gas.moles)
		...
		proc/set_tracegas(var/datum/gas/g,var/amt)
			var/datum/gas/trace_gas = get_tracegas(g)
			if(trace_gas)
				trace_gas.moles = amt
			else
...
//Call it with negative numbers to remove gases.
		...
		proc/adjust(o2 = 0, co2 = 0, n2 = 0, tx = 0, list/datum/gas/traces = list())
			...
			for(var/datum/gas/G in traces)
				var/datum/gas/T = get_tracegas(G)//locate(G.type) in trace_gases
				if(T)
...
////////////////////////////////////////////
		...
		proc/check_tile_graphic()
			...
			if(toxins > MOLES_PLASMA_VISIBLE)
				...
				var/datum/gas/snow = locate(/datum/gas/snow) in trace_gases
				var/datum/gas/rain = locate(/datum/gas/rain) in trace_gases
				var/datum/gas/coolant = locate(/datum/gas/coolant) in trace_gases

				//world << sleeping_agent
				//world << snow
				//world << rain
				//world << coolant
...
//////////////////////////////////////////////
		...
		proc/check_then_merge(datum/gas_mixture/giver)
			...
			if(giver.trace_gases.len)
				for(var/datum/gas/trace_gas in giver.trace_gases)
					var/datum/gas/corresponding = get_tracegas(trace_gas)//locate(trace_gas.type) in trace_gases
		...
		proc/merge(datum/gas_mixture/giver)
			...
			if(giver.trace_gases.len)
				for(var/datum/gas/trace_gas in giver.trace_gases)
					add_tracegas(trace_gas,trace_gas.moles*giver.group_multiplier/group_multiplier)
			update_values()
		...
		proc/remove(amount)
			...
			if(trace_gases.len)
				for(var/datum/gas/trace_gas in trace_gases)
					...
					var/datum/gas/corresponding = removed.add_tracegas(trace_gas,(trace_gas.moles/sum)*amount)
		...
		proc/remove_ratio(ratio)
			...
			if(trace_gases.len)
				for(var/datum/gas/trace_gas in trace_gases)
					...
					var/datum/gas/corresponding = removed.add_tracegas(trace_gas,trace_gas.moles*ratio)
		...
		proc/copy_from(datum/gas_mixture/sample)
			...
			if(sample.trace_gases.len > 0)
				for(var/datum/gas/trace_gas in sample.trace_gases)
					add_tracegas(trace_gas,trace_gas.moles)
			...
			if(sharer.trace_gases.len)
				for(var/datum/gas/trace_gas in sharer.trace_gases)
					if(trace_gas.moles_archived > MINIMUM_AIR_TO_SUSPEND*4)
						var/datum/gas/corresponding = get_tracegas(trace_gas)
						if(corresponding)
		...
		proc/share(datum/gas_mixture/sharer)
			...
			if(trace_gases.len)
				for(var/datum/gas/trace_gas in trace_gases)
					...
					sharer.add_tracegas(trace_gas,delta)
					...
					trace_types_considered += trace_gas
			...
			if(sharer.trace_gases.len)
				for(var/datum/gas/trace_gas in sharer.trace_gases)
					...
					for(var/datum/gas/g in trace_types_considered)
						if(g.is_tracegas(trace_gas))
							skip = 1
							break

					if(skip)
						continue //THIS SHIT IS HACKY AS FUCK BUT I DON'T GIVE A SHIT NOBODY GAVE A SHIT ABOUT TRACEGASES EITHER
			...
			if(trace_gases.len)
				for(var/datum/gas/trace_gas in trace_gases)
					if(trace_gas.moles > MINIMUM_AIR_TO_SUSPEND)
						var/datum/gas/corresponding = sample.get_tracegas(trace_gas)
						if(corresponding)
							if((abs(trace_gas.moles - corresponding.moles) > MINIMUM_AIR_TO_SUSPEND) && \
